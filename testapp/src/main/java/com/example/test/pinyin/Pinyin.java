...
public class Pinyin {
    ...
    public static String toPinyin(String chinese) {
        ...
        for (int i = 0; i < chinese.length(); i++) {
            String key = chinese.substring(i, i + 1);
            byte[] bytes = getBytesGb2312(key);
            if (bytes.length == 2) {
                String value = convert(bytes);
                buffer.append(value == null ? "[UNKNOWN]" : value);
            } else {
                buffer.append(key);
            }
        }
    ...
    private static int ascii(byte[] bytes) {
        int ascii = 0;
        if (bytes == null || bytes.length > 2 || bytes.length <= 0) {
            throw new IllegalArgumentException("illegal argument");
        }
        if (bytes.length == 1) //English
            ascii = bytes[0];
        else if (bytes.length == 2) { //Chinese
    ...
    private static byte[] getBytesGb2312(String character) {
        try {
            return character.getBytes("gb2312");
